...
function print_commit (c) {
  let m
  console.log(`* [\`${c.shortid}\`](https://github.com/stevebritton/veeva/commit/${c.fullid})`)
  if (c.fixes) {
    console.log(`  [#${c.fixes}](https://github.com/stevebritton/veeva/issues/${c.fixes})`)
  } else if (c.prurl && (m = c.prurl.match(/https:\/\/github.com\/([^/]+\/[^/]+)\/pull\/(\d+)/))) {
    let repo = m[1]
    let prid = m[2]
    if (repo !== 'stevebritton/veeva') {
  ...
  // linkify commitids
    .replace(/\b([a-f0-9]{7,8})\b/g, '[`$1`](https://github.com/stevebritton/veeva/commit/$1)')
    .replace(/\b#(\d+)\b/g, '[#$1](https://github.com/stevebritton/veeva/issues/$1)')
  console.log(msg)
  if (c.credit) {
