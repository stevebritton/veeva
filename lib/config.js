...
config.mergeOptions = function(options) {
    ...
    if (localOptions.Error) {
        ...
        localOptions.keyMessages = localOptions.clm.primary.key_messages.map(function(e) {
            ...
            //e.key_message = e.key_message.replace(/ /g,'').trim().toLowerCase();
            return e;
        });
        */

       localOptions.keyMessages = [];

        // add global key message to array
        localOptions.keyMessages.unshift({ 'key_message': 'global' });

        /*
        if(localOptions.clm.assets && localOptions.clm.assets.key_messages){
        ...
        localOptions.module.workflow.assemble.data = {
            ...
            'keyMessages': localOptions.keyMessages,
            ...
            'title': '',
            'root': '/',
            'veevaTrackSubsceneField': localOptions.crm.customFields.veevaTrackSubsceneField || '',
            'version': 1
        };
