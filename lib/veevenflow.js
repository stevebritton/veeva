    ...
    return tracker.askPermissionAndTrack(options).then(function() {
        ...
        if (cliArgs.version) {
            ...
            if (options.deploy.keyMessage) {
                ...
                indexKeyMessageExists = arrMergeKeyMessageNames.indexOf(options.clm.product + '-' + options.deploy.keyMessage.toString().replace(options.clm.product + '-', ''));

                if (indexKeyMessageExists !== -1) {

                    utils.log.note('⤷ Single Key Message Mode: ' + options.clm.product + '-' + options.deploy.keyMessage);

                    options.deploy.keyMessage = arrMergeKeyMessages[indexKeyMessageExists];
                } else {

                    utils.log.error('Invalid Key Message: ' + options.clm.product + '-' + options.deploy.keyMessage);
